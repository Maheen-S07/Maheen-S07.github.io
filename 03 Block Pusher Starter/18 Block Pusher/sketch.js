...
function swap(x1, y1, x2, y2){
  //modify the game board; swap two items 
...
function keyPressed(){
  ...
  if(keyCode === UP_ARROW && playerY > 0){
    //before doing anything else, inspect top neighbor
    if(level[playerY-1][playerX] === 0){ //GRASS
      ...
      //is there roomfor it to be pushed??
      if(playerY - 2 >= 0 && level[playerY-2][playerX] === 0){
        //grass above the chiken -> allowed ot push
  ...
  else if(keyCode === DOWN_ARROW && playerY < height){
    swap(playerX, playerY, playerX, playerY+1);
    playerY++;
